...
const SPEAKING_PRACTICE_TYPES = {
    'speaking-main': { // Generic category for these top-level speaking exercises
        exercises: ['showQuestionPractice', 'showMonologuePractice', 'showRolePlayPractice'],
...
async function showQuestionPractice() {
    ...
    function displayCurrentQuestion() {
        ...
        const feedbackEl = document.getElementById('speaking-feedback'); // Corrected ID
    ...
    function handleSpeakingRecording() {
        ...
        const feedbackDiv = document.getElementById('speaking-feedback'); // Corrected ID
        ...
        const onErrorCallback = (event) => {
            ...
            if (event.error === 'no-speech') errorMsg = currentTranslations.noSpeechDetected || 'No speech detected. Please try again.';
            else if (event.error === 'audio-capture') errorMsg = currentTranslations.micProblem || 'Audio capture problem. Is your microphone working?';
            else if (event.error === 'not-allowed' || event.error === 'service-not-allowed') errorMsg = currentTranslations.micPermissionDenied || 'Microphone permission denied. Please allow microphone access.';
            else if (event.error === 'network') errorMsg = currentTranslations.networkError || 'Network error during speech recognition.';
            else if (event.error === 'aborted') errorMsg = currentTranslations.recognitionAborted || 'Speech recognition aborted.';
            else errorMsg = `${currentTranslations.errorInRecognition || 'Error in recognition'}: ${event.error}`;
        ...
        startPronunciationCheck(questionText, langCode, 'speaking-transcript', 'speaking-feedback', onStartCallback, onResultCallback, onErrorCallback, onEndCallback);
    }
    ...
    resultArea.innerHTML = `
        <div class="speaking-exercise-container exercise-container">
            ...
            <div id="speaking-feedback" class="feedback-area exercise-feedback" style="margin-top: 10px; min-height: 25px; padding: 5px;"></div>
    ...
    if (typeof setupExerciseCompletionFeedbackObserver === 'function') {
        setupExerciseCompletionFeedbackObserver(feedbackEl, 'speaking-main', 'showQuestionPractice', SPEAKING_PRACTICE_TYPES, 120000);
    }
    if (typeof createStandardRandomizeButton === 'function') {
        const randomButton = createStandardRandomizeButton('speaking-main', 'showQuestionPractice', SPEAKING_PRACTICE_TYPES);
        const actionsDiv = resultArea.querySelector('.exercise-actions');
        if (actionsDiv) actionsDiv.appendChild(randomButton);
        else resultArea.querySelector('.speaking-exercise-container').appendChild(randomButton); // Fallback
    }
    ...
    document.getElementById('next-speaking-question-btn').addEventListener('click', () => {
        if (currentQuestionIndex < questions.length - 1) { currentQuestionIndex++; displayCurrentQuestion(); }
    });
...
async function showMonologuePractice() {
    ...
    if (typeof createStandardRandomizeButton === 'function') {
        const randomButton = createStandardRandomizeButton('speaking-main', 'showMonologuePractice', SPEAKING_PRACTICE_TYPES);
        const actionsDiv = resultArea.querySelector('.exercise-actions');
        if (actionsDiv) actionsDiv.appendChild(randomButton);
        else resultArea.querySelector('.speaking-exercise-container').appendChild(randomButton);
    }
}
...
async function showRolePlayPractice() {
    ...
    if (typeof createStandardRandomizeButton === 'function') {
        const randomButton = createStandardRandomizeButton('speaking-main', 'showRolePlayPractice', SPEAKING_PRACTICE_TYPES);
        const actionsDiv = resultArea.querySelector('.exercise-actions');
        if (actionsDiv) actionsDiv.appendChild(randomButton);
        else resultArea.querySelector('.speaking-exercise-container').appendChild(randomButton);
    }
...
async function practiceAllSpeaking() { // This is the main entry for "Practice All Speaking" button
    await startRandomSpeakingPractice(); // It will pick one from the category
}

async function startRandomSpeakingPractice() {
    const category = SPEAKING_PRACTICE_TYPES['speaking-main'];
    if (!category || !category.exercises || category.exercises.length === 0) {
        console.error("Speaking exercises not defined or empty in SPEAKING_PRACTICE_TYPES.");
        return;
    }
    // For now, let's assume all listed exercises are available or placeholders that can be "called"
